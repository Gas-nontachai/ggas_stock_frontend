...
onMounted(async () => {
  try {
    ...
    const res_user = await searchUser();
    users.value = res_user;

    isSold.value = item.value.item_status !== 1;

    if (isSold.value) {
      const soldResponse = await searchIncome({
        where: { item_id },
        include: [
          {
            model: 'platform',
        ...
        <v-col cols="12" md="6">
          <v-carousel v-if="Array.isArray(item.item_image) && item.item_image.length" height="300"
            ...
            <v-carousel-item v-for="(img, index) in item.item_image" :key="index">
              <v-img :src="img" :alt="`${t('image')} ${index + 1}`" cover class="bg-grey-lighten-2"
          ...
          <v-card elevation="2" class="pa-4">
            ...
            <v-card-text>
              <div class="font-weight-medium mb-3">
                {{ t('item.item_price') }}: <span class="text-primary">${{ decimalFix(item.item_buy_price) }}</span>
              </div>
              ...
              <div class="d-flex align-center mb-2">
                ...
                <v-chip :color="item.item_status === 1 ? 'success' : 'error'" text-color="white" size="small">
                ...
                <v-card-text>
                  <v-list>
                    <v-list-item>
                      ...
                      <v-list-item-title>{{ t('income.income_id') }} : {{ income.income_id }}</v-list-item-title>
                    </v-list-item>
                  </v-list>
                  <v-list>
                    <v-list-item>
                      ...
                      <v-list-item-title>{{ t('income.income_sell_price') }} : {{ decimalFix(income.income_sell_price) }}฿</v-list-item-title>
                    </v-list-item>
                  </v-list>
                  <v-list>
                    <v-list-item>
                      ...
                      <v-list-item-title>{{ t('income.profit') }} : {{ decimalFix(income.income_sell_price - item.item_buy_price) }}฿</v-list-item-title>
                    </v-list-item>
                  </v-list>
                  <v-list>
                    <v-list-item>
                      ...
                      <v-list-item-title>{{ t('income.income_note') }} : {{ income.note }}</v-list-item-title>
                    </v-list-item>
                  </v-list>
                  <v-list>
                    <v-list-item>
                      ...
                      <v-list-item-title>{{ t('platform.platform_name') }} : {{ income.tb_platform?.platform_name }}</v-list-item-title>
                    </v-list-item>
                  </v-list>
                </v-card-text>
              </v-window-item>
                ...
                <v-card-text>
                  <v-list>
                    <v-list-item>
                      ...
                      <v-list-item-title>{{ t('item.item_id') }} : {{ item.item_id }}</v-list-item-title>
                    </v-list-item>
                    <v-list-item>
                      <template v-slot:prepend>
                        <v-icon icon="mdi-tag-outline"></v-icon>
                      </template>
                      <v-list-item-title>{{ t('category.category_name') }} : {{ item.tb_category?.category_name }}</v-list-item-title>
                    </v-list-item>
                    <v-list-item>
                      <template v-slot:prepend>
                        <v-icon icon="mdi-currency-thb"></v-icon>
                      </template>
                      <v-list-item-title>{{ t('item.item_price') }}: {{ decimalFix(item.item_buy_price) }} ฿</v-list-item-title>
                    </v-list-item>
                  </v-list>
                </v-card-text>
              </v-window-item>
                ...
                <v-card-text>
                  <v-list>
                    <v-list-item>
                      ...
                      <v-list-item-title>{{ t('item.addby') }} : {{ getUserName(item.addby || '') }}</v-list-item-title>
                    </v-list-item>
                    <v-list-item>
                      <template v-slot:prepend>
                        <v-icon icon="mdi-calendar-clock"></v-icon>
                      </template>
                      <v-list-item-title>{{ t('item.add_date') }} : {{ formatDate(item.createdAt) }}</v-list-item-title>
                    </v-list-item>
                    <v-list-item>
                      <template v-slot:prepend>
                        <v-icon icon="mdi-account-edit"></v-icon>
                      </template>
                      <v-list-item-title>{{ t('item.update_by') }} : {{ getUserName(item.updateby || '') }}</v-list-item-title>
                    </v-list-item>
                    <v-list-item>
                      <template v-slot:prepend>
                        <v-icon icon="mdi-calendar-edit"></v-icon>
                      </template>
                      <v-list-item-title>{{ t('item.update_date') }} : {{ formatDate(item.updatedAt) }}</v-list-item-title>
                    </v-list-item>
                  </v-list>
                </v-card-text>
              </v-window-item>
