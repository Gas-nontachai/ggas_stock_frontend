...
const item_per_page = ref(12);
const page_length = ref(0);
const page = ref(1);
const paginatedItems = computed(() => {
  const start = (page.value - 1) * item_per_page.value;
  const end = start + item_per_page.value;
  return props.items.slice(start, end);
});

onMounted(() => {
  try {
    page_length.value = Math.ceil(props.items.length / item_per_page.value);
  } catch (error) {
    console.error('Error calculating pagination:', error);
    Swal.fire({
      icon: 'error',
      title: 'Error',
      text: 'Failed to load item data. Please try again later.',
    });
  } finally {
    loading.value = false;
  }
    ...
    <v-row v-else>
      <v-col cols="12">
        <div class="text-center">
          <v-pagination v-model="page" :length="page_length" :total-visible="4" color="primary"></v-pagination>
        </div>
      </v-col>

      <v-col v-for="item in paginatedItems" :key="item.item_id" cols="12" sm="6" md="4" lg="3">
        <v-card class="h-100">
          <div :class="{ 'opacity-50': props.status === 0 }">
            <v-carousel v-if="Array.isArray(item.item_image) && item.item_image.length" height="300"
              ...
              <v-carousel-item v-for="(img, index) in item.item_image" :key="index">
                <v-img :src="img" :alt="`Image ${index + 1}`" cover class="bg-grey-lighten-2"
          ...
          <div :class="{ 'opacity-70': props.status === 0 }">
